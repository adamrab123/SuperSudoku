...
def parse_grid(path):
	...
	infile = open("knn.jpg", "rb")
	knn = pickle.load(infile)
	nonempty_digits = [d for d in digits if np.sum(d)>0]
	x = np.array([hog(d, orientations=8, pixels_per_cell=(8,8), cells_per_block=(3,3)) for d in nonempty_digits])
	y = knn.predict(x)
	nums = np.zeros(81)
	j = 0
	for i in range(81):
	...
	# print(nums)
	# for
	np.savetxt("file.txt", nums.reshape([9,9]), fmt='%d', delimiter = "")

def main():
	parse_grid('sudoku1.png')
